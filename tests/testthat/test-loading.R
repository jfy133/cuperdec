...
table_fail <- readr::read_tsv(raw_table) %>% dplyr::select(`#Datasets`)
...
testthat::test_that("Isolation source database table is correctly
  ...
  testthat::expect_named(converted_database, c("Taxon", "Isolation_Source"))
})


database_fail <- readr::read_tsv(raw_database) %>% dplyr::mutate(foo = "bar")
database_broken <- readr::read_tsv(raw_database) %>% dplyr::mutate(isolation_source = "bar")
...
converted_metadata <- readr::read_tsv(raw_metadata) %>%
  cuperdec::load_map(x = ., sample_col = "#SampleID", source_col = "Env")
...
testthat::test_that("Metadata map table is correctly
  ...
  testthat::expect_named(converted_metadata, c("Sample", "Sample_Source"))
})

map_fail_insufficientcols <- readr::read_tsv(raw_metadata) %>% dplyr::select(`#SampleID`)
map_fail_missingnamecol <- readr::read_tsv(raw_metadata) %>% dplyr::rename("Foo" = `#SampleID`)
map_fail_missingsourcecol <- readr::read_tsv(raw_metadata) %>% dplyr::rename("Foo" = "Env")

testthat::test_that("Fails when metadata file broken", {
  testthat::expect_error(load_map(map_fail_insufficientcols, "#SampleID", "Env"))
  testthat::expect_error(load_map(map_fail_missingnamecol, "#SampleID", "Env"))
  testthat::expect_error(load_map(map_fail_missingsourcecol, "#SampleID", "Env"))
})
